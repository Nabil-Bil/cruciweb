...
<body>
  ...
  <div class="center-content">
    <form action="${pageContext.request.contextPath}/register" method="post" id="auth-form" class="card">
      ...
      <input type="text" name="username" id="username" required class="${not empty username_error ? 'error-input' : ''}"/>
      <span class="${not empty password_error ? 'error' : 'hidden'}">${password_error}</span>
      <label for="password">Mot de passe</label>
      <input type="password" name="password" id="password" required class="${not empty password_error or not empty password_confirmation_error ? 'error-input' : ''}" />
      <span class="${not empty password_confirmation_error ? 'error' : 'hidden'}">${password_confirmation_error}</span>
      <label for="confirm_password">Confirmer mot de passe</label>
      <input type="password" name="confirm_password" id="confirm_password" required class="${not empty password_confirmation_error ? 'error-input' : ''}"  />
